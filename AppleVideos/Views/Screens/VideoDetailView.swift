            ...
            if let url = video.youtubeURL {
                ToolbarItemGroup(placement: .topBarTrailing) {
                    ...
                    if case let .downloading(fraction) = TestDownloads.shared.states[video.id] {
                        ...
                        Menu {
                            ForEach([TestDownloads.Mode.youtube, .youtubeWithoutSubtitles, .appleSample, .youtubeViaProxy, .youtubeH264, .youtubePlaylistViaApp], id: \.self) { mode in
                            ...
                            Button("Send remote test") {
                                TestDownloads.shared.sendRemoteTest()
                            }
