...
Native playback presents `AVPlayerViewController` modally from a transparent UIKit host attached to the video detail screen. The player retains AVKit's default modal presentation style, view background, and opacity so AVKit owns the backdrop, video controls, and interactive dismissal. The app does not animate a second black surface during dismissal or place a black loading screen behind AVKit. The Play button indicates source resolution before the player exists; AVKit handles loading and buffering once the stream reaches the player. The embedded YouTube fallback retains its own full-screen surface. The host handles Picture in Picture restoration and returns to the video detail screen after dismissal. There are no custom native player gestures or controls. Metadata handoff, background audio, and the cellular HLS quality preference remain in place.
