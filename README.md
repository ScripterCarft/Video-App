...
- Watch progress is saved per video on device (`LibraryStore`, `apple-videos.progress`, up to 200 videos): every five seconds while playing, and immediately on pause, at the end, when the player closes and when the app enters the background. After a crash at most the last few seconds are lost. Positions under ten seconds or past 95 % are not kept. Play resumes at the saved position once the item is ready, and the detail screen shows a progress bar and the remaining time next to Play. During playback progress is only written to storage; the UI picks it up when the player closes.
