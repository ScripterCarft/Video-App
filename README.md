...
Temporary device diagnostic: the video detail screen background is black on the left and transparent on the right. This isolates whether the detail screen's own black background is visible during native swipe dismissal. Remove the split after checking it on an iPhone.

Native playback presents `AVPlayerViewController` modally from a transparent UIKit host attached to the video detail screen. The presentation uses `overFullScreen` so UIKit keeps that detail screen in its view hierarchy throughout an interactive dismissal. The player's outer view is transparent and nonopaque, leaving the visible backdrop and swipe animation to AVKit. The app does not animate a second black surface during dismissal or place a black loading screen behind AVKit. The Play button indicates source resolution before the player exists; AVKit handles loading and buffering once the stream reaches the player. The embedded YouTube fallback retains its own full-screen surface. AVKit owns the video controls and full-screen dismissal. The host handles Picture in Picture restoration and returns to the video detail screen after dismissal. There are no custom native player gestures or controls. Metadata handoff, background audio, and the cellular HLS quality preference remain in place.
