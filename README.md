...
- `Views/Player/`: the native tab accessory mini player and embedded YouTube fallback (SwiftUI)
...
- Watch progress is saved per video on device (`LibraryStore`, in SwiftData, up to 200 videos): every five seconds while playing, and immediately on pause, at the end, when the player closes and when the app enters the background. After a crash at most the last few seconds are lost. Positions under ten seconds or past 95 % are not kept. Play resumes at the saved position: the seek is issued as soon as the item is ready and playback starts only after it finishes, so the first frame shown is the saved position, and Play buttons show the play symbol, a progress gauge and the remaining time instead of the word Play. Home's Featured and the detail hero share the same Play-button configuration. Full-screen playback defers publishing progress until dismissal; mini playback publishes saved updates while the library is visible.
- Dismissal is reported by `playerViewController(_:willEndFullScreenPresentationWithAnimationCoordinator:)`. A completed dismissal minimizes the existing session into UIKit's `UITabAccessory`. Cancelled gestures leave it alone. The accessory shows thumbnail/title, Play/Pause and Close. Expanding presents the same controller and player, preserving position and paused state. Closing the mini player or PiP ends the session once. Restoring from PiP presents the same controller again.
